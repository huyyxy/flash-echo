"""Generic causal-LM SFT trainer for short filler-prefix data.

This module contains the shared implementation used by model-specific entry
points such as ``train_minimind3_sft.py`` and ``train_qwen3_5_sft.py``.
...
Loss is computed only on assistant reply tokens and the assistant end-of-turn
marker, so the model is supervised to stop after the short prefix.
"""
...
class FillerPrefixSFTDataset(Dataset[tuple[torch.Tensor, torch.Tensor]]):
    ...
    def _assistant_spans(
        ...
        cursor = 0
        end_markers = self._assistant_end_markers()
        for message in conversations:
            ...
            end = self._include_assistant_end_marker(rendered_chat, end, end_markers)
            spans.append((start, end))
            cursor = end
        return spans

    def _assistant_end_markers(self) -> tuple[str, ...]:
        markers: list[str] = []
        ...
        if isinstance(special_map, dict):
            for value in special_map.values():
                if isinstance(value, str):
                    ...
                    markers.extend(item for item in value if isinstance(item, str))
        ...
        # Prefer longer markers first so ChatML-style tokens win over short overlaps.
        unique = sorted({marker for marker in markers if marker}, key=len, reverse=True)
        return tuple(unique)

    @staticmethod
    def _include_assistant_end_marker(
        rendered_chat: str,
        content_end: int,
        end_markers: tuple[str, ...],
    ) -> int:
        for marker in end_markers:
            if rendered_chat.startswith(marker, content_end):
                return content_end + len(marker)
        return content_end
...
def parse_args(*, description: str) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--base-model",
        default=None,
        help="Pretrained model path or Hugging Face id.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Checkpoint root. Defaults to the model-specific entrypoint path.",
    )
    parser.add_argument(
        "--model-version",
        default=None,
        help="Recorded in model_card.json. Defaults to the model-specific entrypoint version.",
    )
...
def validate_args(
    ...
    *,
    resolve_paths: Callable[[argparse.Namespace], None],
) -> None:
    resolve_paths(args)
...
def main(
    *,
    ...
    resolve_paths: Callable[[argparse.Namespace], None],
    template_dependency_name: str,
) -> int:
    try:
        import jinja2  # noqa: F401
    except ImportError as exc:
        raise ImportError(
            f"{template_dependency_name} requires jinja2. Install with: pip install jinja2"
        ) from exc

    args = parse_args()
    validate_args(args, resolve_paths=resolve_paths)
