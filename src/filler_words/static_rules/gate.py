    ...
    def from_config(cls, payload: dict[str, Any]) -> StaticReplyGate:
        ...
        if isinstance(raw_rules, list):
            for item in raw_rules:
                ...
                patterns = tuple(match_cfg.get("patterns", [])) if isinstance(match_cfg, dict) else ()
                replies = item.get("replies", {})
                if not patterns or not isinstance(replies, dict):
                    continue
                normalized_replies = {
    ...
    def _matches_rule(normalized_query: str, rule: StaticRule) -> bool:
        for negative in rule.negative_examples:
            if normalize_query(negative) == normalized_query:
                return False
        return normalized_query in {normalize_query(pattern) for pattern in rule.patterns}
    ...
    "rules": [
        {
            ...
            "category": "greeting",
            "match": {"type": "exact_or_alias", "patterns": ["你好", "您好", "哈喽", "嗨"]},
            "negative_examples": [],
            "replies": {
        ...
        },
        {
            ...
            "category": "thanks",
            "match": {"type": "exact_or_alias", "patterns": ["谢谢", "多谢", "辛苦了"]},
        ...
        },
        {
            ...
            "category": "confirm",
            "match": {"type": "exact_or_alias", "patterns": ["好的", "好", "嗯", "行", "可以"]},
        ...
        },
        {
            ...
            "category": "farewell",
            "match": {"type": "exact_or_alias", "patterns": ["再见", "拜拜", "下次聊"]},
            "negative_examples": [],
            "replies": {
